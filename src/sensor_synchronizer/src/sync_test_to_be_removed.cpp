...
void MatchGrabber::Callback(const sensor_msgs::ImageConstPtr &msgLeft, const sensor_msgs::ImageConstPtr &msgRight){
  ...
  if (!hit_flag){
    if(Left_img_sec + Left_img_nsec/1e9-Left_last_timestamp > 0.040 && (counter%recording_image_rate==0)){
        ...
        //cout << imLeft << endl;
        cv::Size s = imLeft.size(); //768*1024
        int size_t = imLeft.total(); //786432
        int t = imLeft.channels(); //3
        std::vector<uint16_t> array;
        if (imLeft.isContinuous()) {
            array.assign(imLeft.datastart, imLeft.dataend);
        }
        rover_status.image_l_mat = array;
        cout << "--------------------------------------------------------------"<<endl;
        cout << array.size() << endl;
        //cout << static_cast<unsigned int>(array[0]) << std::flush;
